...
const AddHostModal = React.lazy(() => import('./components/AddHostModal'))
const BrowserPane = React.lazy(() => import('./components/BrowserPane'))
const EditorPane = React.lazy(() => import('./components/EditorPane'))
const SftpCommander = React.lazy(() => import('./components/SftpCommander'))
const ExportImportModal = React.lazy(() => import('./components/ExportImportModal'))
...
export default function App() {
  ...
  // ─── НОВЫЕ ОПТИМИЗИРОВАННЫЕ ОБРАБОТЧИКИ ДЛЯ TABBAR ──────────────────────────
  const handleOpenSettings = useCallback(() => setShowSettings(true), [])
  const handleOpenTunnels = useCallback(() => setShowTunnels(true), [])
  const handleGoHome = useCallback(() => {
    const welcome = tabs.find(t => t.type === 'welcome')
    if (welcome) {
      setActiveTab(welcome.id)
    } else {
      setActiveTab(null)
    }
  }, [tabs])

  const handleToggleX11 = useCallback(async () => {
    const next = !x11
    setX11(next)
  ...
  const handleToggleMonitor = useCallback(() => {
    setMonitorEnabled(v => !v)
  }, [])

  const handleSetTabColor = useCallback((tabId, color) => {
  ...
  const sidebarProps = useMemo(() => ({
    ...
    compact, onHideSidebarCb
  ])

  // ─── ОПТИМИЗИРОВАННЫЕ ПРОПСЫ ДЛЯ TABBAR (НОВОЕ) ──────────────────────────
  const tabBarProps = useMemo(() => ({
    ...
    x11,
    onToggleX11: handleToggleX11,
    monitor: monitorEnabled,
    onToggleMonitor: handleToggleMonitor,
  }), [
    tabs, activeTab, handleSelect, closeTab, handleOpenSettings,
    handleOpenTunnels, autoTunnels, handleSetTabColor, handleGoHome,
    openSftpCommander, x11, handleToggleX11, monitorEnabled,
    handleToggleMonitor
  ])
...
return (
    <div style={{ display: 'flex', height: '100vh', minHeight: 0, overflow: 'hidden', background: 'var(--bg0)' }}>
      ...
      {showSettings && (<Suspense fallback={null}><SettingsModal settings={termSettings} onSave={saveSettings} onClose={() => setShowSettings(false)} /></Suspense>)}

      {showTunnels && (<Suspense fallback={null}><TunnelRulesModal rules={tunnelRules} onSave={saveTunnelRules} onClose={() => setShowTunnels(false)} /></Suspense>)}

      {showExportImport && (<Suspense fallback={null}><ExportImportModal
          hosts={hosts}
          groups={customGroups}
      ...
      {showAddHost && (<Suspense fallback={null}><AddHostModal
